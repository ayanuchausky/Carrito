...
export class ClienteService {
  ...
  clienteLogueado(): Observable<EntityResponseType>{
    return this.http.get<ICliente>(`${this.resourceUrl}/actual`, { observe: 'response' });
  }

  historialVip(mes: number):Observable<EntityArrayResponseType> {
    return this.http.get<ICliente[]>(`${this.resourceUrl}/vip/${mes}`,{ observe: 'response' });
  }
  historialComun(mes: number):Observable<EntityArrayResponseType> {
    return this.http.get<ICliente[]>(`${this.resourceUrl}/comun/${mes}`,{ observe: 'response' });
  }

  clientesVip(): Observable<EntityArrayResponseType>{
    return this.http.get<ICliente[]>(`${this.resourceUrl}/vip`,{ observe: 'response'});
  }
}
